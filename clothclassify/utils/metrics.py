...
def evaluate_rank(distmat: np.ndarray, pids: np.ndarray, ranks):
    assert distmat.shape[0] == distmat.shape[1]
    assert distmat.shape[0] == len(pids)
    ...
    for rank in ranks:
        correct_count = 0
        for i in range(N):
            distances = distmat[i].copy()
            
            distances[i] = np.inf  # Исключаем сам объект (расстояние до себя)
            sorted_indices = np.argsort(distances)
            
            top_k_indices = sorted_indices[:rank]
            
            if np.any(pids[top_k_indices] == pids[i]):
                correct_count += 1
